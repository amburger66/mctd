...
class DiffusionForcingPlanning(DiffusionForcingBase):
    ...
    def _decide_children_stable_prefix(self, node):
        remaining = node.plan_tokens - node.tokens_committed
        if remaining <= 0:
            return 0
        # TODO: from node.value_estimation_plan, detect contiguous low-multimodality
        # leading region and return its length. For now, fully autoregressive.
        return 4
        ...
        while True:
            ...
            for _ in range(
                ...
                for info in expanded_node_candidates:
                    ...
                    c = info["tokens_committed"] - info["stable_prefix"]  # tokens at 0 entering this expansion
                ...
                for i, nl in enumerate(expanded_node_noise_levels):
                    if nl.shape[0] < max_len:
                        ...
                        expanded_node_noise_levels[i] = np.concatenate([nl, pad], axis=0)
                expanded_node_noise_levels = np.array(expanded_node_noise_levels, dtype=np.int32)
                ...
                for i in range(
                    ...
                    c_new = expanded_node_candidates[i]["tokens_committed"]  # already includes this expansion's commit
                    _noise_level = self._build_valuation_slab(c_new, T, plan_tokens, skip_level_steps)
            ...
            for name in selected_nodes_for_expansion:
                child = selected_nodes_for_expansion[name].expand(**expanded_node_infos[name])
                # TODO: this is a dummy function and should be determined based on multimodality estimate from simulation step
                child.children_stable_prefix = self._decide_children_stable_prefix(child)
